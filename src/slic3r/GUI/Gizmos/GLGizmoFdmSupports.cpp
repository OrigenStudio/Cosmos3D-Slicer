...
void GLGizmoFdmSupports::on_render_input_window(float x, float y, float bottom_limit)
{
    ...
    for (int i = 0; i < tool_ids.size(); i++) {
        ...
        if (m_current_tool == tool_ids[i]) {
            ImGui::PushStyleColor(ImGuiCol_Button,          ImVec4(0.28f, 0.45f, 0.89f, 0.25f));  // Equivalent to #4872E3
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered,   ImVec4(0.28f, 0.45f, 0.89f, 0.25f)); // ORCA use orca color for selected tool / brush
            ImGui::PushStyleColor(ImGuiCol_ButtonActive,    ImVec4(0.28f, 0.45f, 0.89f, 0.30f));  // ORCA use orca color for selected tool / brush
