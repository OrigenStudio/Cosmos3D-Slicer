...
bool run_cosmos_post_processing(const std::string &gcode_path, const DynamicPrintConfig &config)
{
    ...
    try {
        ...
        while (std::getline(input_file, line)) {
            ...
            // Add custom Cosmos3D-specific processing
            
            // 1. Add Cosmos3D printer identification comment at the beginning
            if (line.find(";FLAVOR:") != std::string::npos || 
                line.find("; generated by") != std::string::npos) {
                processed_gcode += line + "\n";
                processed_gcode += "; Post-processed for Cosmos3D printer\n";
                processed_gcode += "; Cosmos3D optimizations applied\n";
                continue;
            }

            // 2. Detect start of actual printing
            if (line.find("G1") == 0 && line.find("E") != std::string::npos && !in_print_section) {
                in_print_section = true;
                processed_gcode += "; === Cosmos3D Print Section Start ===\n";
                // Add custom start G-code for Cosmos3D
                processed_gcode += "M117 Cosmos3D Printing...\n"; // Display message
            }

            // 3. Optimize temperature commands for Cosmos3D
            if (line.find("M104") == 0 || line.find("M109") == 0) {
                // Add temperature stability comment for Cosmos3D
                processed_gcode += "; Cosmos3D temperature optimization\n";
            }

            // 4. Add custom cooling fan control for Cosmos3D
            if (line.find("M106") == 0) {
                processed_gcode += "; Cosmos3D fan control optimization\n";
            }

            // 5. Add layer change notifications for Cosmos3D
            if (line.find(";LAYER_CHANGE") != std::string::npos) {
                processed_gcode += line + "\n";
                processed_gcode += "M117 Layer changing...\n"; // Cosmos3D layer change notification
                continue;
            }

            // 6. Add end-of-print processing for Cosmos3D
            if (line.find("M84") == 0 || line.find("M18") == 0) {
                if (in_print_section) {
                    processed_gcode += "; === Cosmos3D Print Section End ===\n";
                    processed_gcode += "M117 Cosmos3D Print Complete!\n"; // Final message
        ...
        // Write the processed G-code back to the file
